...
- Done: `_solve()` now uses `Isolate.run(...)`; a solver failure surfaces as a SnackBar instead of leaving the spinner stuck.
...
- `lib/widgets/graph_2d.dart` — `Graph2DView`. Adaptive sampling (220 base cells + curvature refinement to depth 4), gap detection at discontinuities, pan + pinch-zoom, double-tap auto-fit, robust Y auto-fit (2% tail trim), 1-2-5 nice grid ticks + axis labels. Sampling runs in `Isolate.run`.
- `lib/widgets/graph_3d.dart` — `Graph3DView`. `z = f(x, y)` mesh (42×42 over `[-6, 6]²`), undefined cells skipped, drag-rotate + pinch-zoom, painter's-algorithm depth sort, height ramp `#4D4DA6 → #00E5AA` with normal-based shading, axes labels + height legend. Sampling in `Isolate.run`.
